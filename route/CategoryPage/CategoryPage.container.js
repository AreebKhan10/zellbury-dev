...
export class CategoryPageContainer extends PureComponent {
    ...
    getSelectedSortFromUrl() {
        const { storeConfigurations } = BrowserDatabase.getItem('config') || {
            storeConfigurations: []
        };

        let storeSortKey = '';
        if (storeConfigurations) {
            storeConfigurations.map(({identifier, label}, i) => {
                if(identifier == 'default_sort_key')
                {
                    storeSortKey = label;
                }
            })
        ...
        const configuredSortKey = storeSortKey || globalDefaultSortKey;
        const defaultSortKey = default_sort_by || configuredSortKey;
