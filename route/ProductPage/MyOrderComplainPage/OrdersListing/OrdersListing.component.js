...
export class OrdersListing extends Component {
    ...
    getCustomerOrders = async () => {
        ...
        if (itemOrders.length) {
            ...
            let active = itemOrders.filter(item => item.status_label.toLowerCase() != "canceled_inventory_mismatch" && item.status_label != "delivered" && item.status_label != "complete" && item.status_label != "closed" && item.status_label != "canceled" && item.status_label != "canceled_reordered" && item.status_label.toLowerCase() != "undelivered");
            ...
            if (activeTabData.orderId && activeTabData.orderOption && activeTabData.orderOption === "cancelMyOrder" && targetVal || activeTabData.orderId && activeTabData.orderOption && activeTabData.orderOption === "changeMyOrder" && targetVal) {
                ...
                if (targetVal === "wrongOrderAddress") {
                    ...
                    if (obj.orderData.status_label === "pending" || obj.orderData.status_label === "confirmed" || obj.orderData.status_label === "picking" || obj.orderData.status_label === "processing") {
                        ...
                        detailData["deliveryDetail"] = `I'm afraid it's a little too late to cancel your order now as it is already shipped by our courier. Expect to receive your order at your doorstep at (${obj?.orderData?.estimated_delivery})\n\nAny order cancellations at this point will not be refunded if it's online paid or in the case of COD you might be prevented from using COD in future.`;
                ...
                if (targetVal === "itemFaster") {
                    ...
                    detailData["deliveryDetail"] = `${obj.customer_info.shipping_address.firstname.split(' ')[0].trim()} your order is delayed. Our teams are already in touch with the courier partner Leoperd to ensure you get your order. \n\nWe apologize for inconvenience you can see the detailed tracking below.`;
                    // detailData["courierMsg"] = `We will contact our courier partner ${obj.customer_info.shipping_description.split('-')[0]} to deliver your order at your order faster`;
                    // detailData["msg"] = "You can see the detailed tracking below";
                    detailData["detailTracking"] = true;
                    detailData["submit"] = true;
