    ...
    submitTicketWithImage = (ticketType) => {
        if (countForTicket === 0) {
            ...
            const promise1 = new Promise((resolve, reject) => {
                let tagsArr = ["101", `${complainData?.barcodeData?.data?.validateBarcode?.order[0]?.source_name}`, `${complainData?.barcodeData?.data?.validateBarcode?.order[0]?.city}`]
        ...
        if (countForTicket === 0) {
            ...
            if (orderMainOpt) {
                if (orderMainOpt === "wrongProduct") {
                    ...
                    dataObj["source"] = 101;
                    dataObj["tags"] = ["101", `${complainData?.barcodeData?.data?.validateBarcode?.order[0]?.source_name}`, `${complainData?.barcodeData?.data?.validateBarcode?.order[0]?.city}`];
