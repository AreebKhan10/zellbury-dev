...
export class ProductPageContainer extends PureComponent {
    ...
    requestWishlistCount(id) {
        var myHeaders = new Headers();
        myHeaders.append("Authorization", "Bearer 4bndenqr0v1r62qox28s60lir7911tzt");
        myHeaders.append("Content-Type", "application/json");
        var graphql = JSON.stringify({
            query: `{\ncustomerWishlistCount(product_id: \"${id}\") {\ncount_wishlist_product\n}\n}`,
            variables: {}
        })
        var requestOptions = {
            method: 'POST',
    ...
    requestProductSalable(sku) {
        if (sku) {
            var myHeaders = new Headers();
            myHeaders.append("Content-Type", "application/json");
            var graphql = JSON.stringify({
                query: `{\n  productSalableGet(sku: \"${sku}\") {\n    items {\n      id\n      sku\n      salable\n    }\n  }\n}`,
                variables: {}
            })
            var requestOptions = {
                method: 'POST',
            ...
            fetch(`${window.location.origin}/graphql`, requestOptions)
                .then(response => response.text())
                .then(result => {
                    let product_sc = JSON.parse(result);
                    if (product_sc.data) {
                        if (product_sc.data.productSalableGet) {
                            this.setState({ scaleableItems: product_sc.data.productSalableGet.items });
                        }
                        else {
                            this.setState({ scaleableItems: [] });
                        }
    ...
    updateHeaderState() {
        const { name = '', attributes, id, sku } = this.getDataSource();
        const { changeHeaderState } = this.props;
        this.requestWishlistCount(id);
        this.requestProductSalable(sku);
        let Pieces = '';
        let Fabric = '';
        if (attributes && (Object.keys(attributes).length > 0 || attributes.length > 0)) {
            if (Object.keys(attributes).length > 0) {
                if (attributes['pieces'] && attributes['pieces'].attribute_options && Object.keys(attributes['pieces'].attribute_options).length > 0) {
                    let options = attributes['pieces'].attribute_options;
                    let key = Object.keys(options)[0];
                    Pieces = attributes['pieces'].attribute_options[key].label
                }
                if (attributes && attributes['fabric'] && attributes['fabric'].attribute_options) {
                    let options = attributes['fabric'].attribute_options;
                    let key = Object.keys(options)[0];
                    Fabric = attributes['fabric'].attribute_options[key].label
                }
            }
            else {
                Pieces = attributes.find(x => x.attribute_label.toLowerCase() == 'pieces').attribute_options[0].label;
                Fabric = attributes.find(x => x.attribute_label.toLowerCase() == 'fabric').attribute_options[0].label;
            }
        }
        changeHeaderState({
            name: PDP,
