...
export class Checkout extends PureComponent {
    static propTypes = {
        ...
        onLocationHandler: PropTypes.func.isRequired,
        withoutLocation: PropTypes.func.isRequired,
    ...
    componentDidMount() {
        ...
        console.log('checkout component did mount........', this.props);
    ...
    dataLayerHandler() {
        ...
        if (count === 1) {
            try {
                ...
                const GtmDataLayerProductArgs1 = {
                    dataLayer: {
                        ...
                        ecommerce: {
                            ...
                            payment_type: this?.props?.partialOrder[0]?.payment_info?.method,
                            items: [...itemsArr1]
                        }
                    },
                };
                ...
                const GtmDataLayerProductArgs = {
                    dataLayer: {
                        ...
                        ecommerce: {
                            ...
                            affiliation: "Zellbury Store",
                            value: this.props?.totals?.subtotal_with_discount, //25.42,
                            tax: this.props?.paymentTotals?.tax_amount || 0,//4.90,
                            shipping: this.props?.paymentTotals?.shipping_amount || 0, //25.42,
                            currency: this.props.totals.quote_currency_code,//"USD",
                            coupon: this.props?.totals?.coupon_code,
                            items: [...itemsArr]
                        }
                ...
                console.log('error from datalayer.......', error);
            }
        }
    ...
    renderPartialDetailsStep() {
        ...
        console.log('renderPartialDetailsStep /success path........', this.props);
        ...
        if (navigator.permissions && navigator.permissions.query) {
            return (
                <main block="Checkout">
                    ...
                    {!this.props.locationState && this.check === 'prompt' && !this.props.iDontWantFastDelievery ? (
                        // <div style={{ position: "absolute", backgroundColor: 'lightgrey', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                        //     <h1>hello world</h1>
                        //     <br />
                        //     <br />
                        //     <button onClick={() => this.props.onLocationHandler()} style={{ backgroundColor: 'grey', padding: '20px', borderRadius: '5px' }}>Click for on the location.</button>
                        //     <br />
                        //     <br />
                        //     <button onClick={() => this.props.withoutLocation()} style={{ backgroundColor: 'grey', padding: '20px', borderRadius: '5px' }}>Continue this process without location.</button>

                        // </div>
                        <div className="location-popup-main">
                            <div className="location-popup-inner">
                                <div className="content">
                                    <h4 className="title">Why we take Location?</h4>
                                    <p className="text">To ensure you get the best order experience we use your location to optimize delivery routes. By allowing location our delivery riders can get to you faster</p>
                                </div>
                                <div className="btn-wrapper">
                                    <button className="btn dark-btn" onClick={() => this.props.onLocationHandler()}>Enable Location</button>
                                    <button className="btn light-btn" onClick={() => this.props.withoutLocation()}>I Don't want fast delivery</button>
                                </div>
                            </div>
                            <div className="location-popup-footer">
                                <p className="text">
                                    <b>Note:</b> In case you have previously blocked your location on Chrome/Browers. Please click the below link
                                </p>
                                <a href="#" className="link" onClick={() => this.setState({ isOpen: true })}>How to enable location</a>
                                {this.state.isOpen && (
                                    <Lightbox
                        ...
                        // (!this.props.locationState && this.check === 'denied' && this.check !== 'prompt' ? (
                        //         <div style={{ position: "absolute", backgroundColor: 'lightgrey', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                        //             <h1>Please select the location mode</h1>
                        //             <br/>
                        //             <br/>
                        //             <button onClick={() => {}} style={{backgroundColor: 'grey', padding: '20px', borderRadius: '5px'}}>How to set location mode or how to enable or disable location on your browser.</button>
                        //         </div>
                        //     ) : 
                        (
                            <ContentWrapper
